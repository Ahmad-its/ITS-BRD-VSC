...
do          
			...
			; Sicherstellen, dass keine Bits über 16 Bit hinausgehen
            LDR     R6, =0xFFFF
            AND     R4, R4, R6
			...
			; 4. Zirkularer Shift des 16-Bit-Musters nach rechts
            AND     R6, R4, #1      ; Unterstes Bit (LSB) isolieren
            LSR     R4, R4, #1      ; Muster um 1 nach rechts schieben
            LSL     R6, R6, #15     ; Isoliertes LSB auf die Position 15 (MSB) schieben
            ORR     R4, R4, R6      ; Beides wieder zusammenfügen (Rotation)
...
forever 
        MOV     R0, #0x8000   ; beliebig änderbar 
        MOV     R1, #16         ; beliebig änderbar 
        BL      Lauflicht     
        BAL     forever         
        ENDP
